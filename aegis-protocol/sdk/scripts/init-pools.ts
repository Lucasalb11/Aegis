...
function loadMints(secretsDir: string): MintInfo[] {
  const file = path.join(secretsDir, "mints.json");
  const parsed: any[] = JSON.parse(fs.readFileSync(file, "utf8"));
  return parsed.map((m) => ({
    ...m,
...
function mintMap(mints: MintInfo[]) {
...
async function consolidateToVaultOwner(
...
async function main() {
  ...
  const wallet = new KeypairWallet(vaultOwner);
  const provider = new AnchorProvider(connection, wallet, {
    commitment: "confirmed",
  });
  const programId = new PublicKey(idl.address);
  const coder = new BorshCoder(idl as any);
  // Use slim IDL (accounts stripped) to bypass account namespace build issues, but keep full coder.
  const slimIdl = { ...(idl as any), accounts: [] as any[] };
  const program = new Program(slimIdl as any, programId, provider, coder);

  // 1) Consolidate all distributed tokens back to the vault owner
  ...
  // 2) Define pools and distribute full supply with randomness
  ...
  // Track remaining supply per mint (all base units)
  const supplyRemaining: Record<string, bigint> = {};
  for (const mint of mints) {
  ...
  // Precompute pools per mint
  const poolsPerMint: Record<string, number[]> = {};
  ...
  const poolAlloc: Record<
    number,
    { amountA: bigint; amountB: bigint }
  > = {};

  // For each mint, allocate across pools that use it
  for (const [sym, remaining] of Object.entries(supplyRemaining)) {
    ...
    poolIdxs.forEach((poolIdx, j) => {
      ...
      const allocEntry =
        poolAlloc[poolIdx] ??
        { amountA: BigInt(0), amountB: BigInt(0) };
      if (sym === symA) {
  ...
  // 3) Initialize pools and add liquidity
  for (let i = 0; i < pools.length; i++) {
    ...
    const alloc = poolAlloc[i] ?? {
      amountA: BigInt(0),
      amountB: BigInt(0),
    };
    const amountA =
      alloc.amountA > BigInt(0) ? alloc.amountA : BigInt(1_000_000);
    const amountB =
      alloc.amountB > BigInt(0) ? alloc.amountB : BigInt(1_000_000);
